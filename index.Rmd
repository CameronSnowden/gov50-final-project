...
cleanWVU2020 <- WVU2020[-c(1,2,3,4,5,6,7),] |>
  rename("CTYNAME" = "Unemployment.Rates..2020..West.Virginia",
         ...
         "EmployedPop" = "X.1",
         "UnemployedPop" = "X.2",
         "UnemploymentRate" = "X.3",
         "UnemploymentRateAsPctOfUSAvg" = "X.4")
...
temp <- cleanWVU2020 |>
  ...
  mutate(pctPopchange =
           as.integer(`POPESTIMATE042020`) /
           as.integer(`CENSUS2010POP`)) 

temp[,5] = as.numeric(sub("%", "", temp$UnemploymentRate, fixed=TRUE))/100

lm(pctPopchange ~ UnemploymentRate, data = temp) |>
  summary()

table1 <- lm(pctPopchange ~ UnemploymentRate, data = temp) |>
  broom::tidy() |>
